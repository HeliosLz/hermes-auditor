...
## 2026-06-13 · Day 9 — discovery 接真全网搜索(HERMES_DISCOVERY=staged|web)

输入侧最后一块"假"补真:web facet 的语料可换成实时全网搜索结果。

- 路径:gpt-5.5 服务端 `web_search`(经 ai.input.im 网关)—— 本地零出网(DuckDuckGo 等直连被区域封锁,实测 000),不加依赖,fetch 调用只带 web_search 一个工具(quarantine 同款)。
- 接缝:`plan/websearch.py`(`HERMES_DISCOVERY`,默认 staged)+ `llm.complete_with_web_search`;只换 `web_untrusted` facet,registry/official 仍 staged(权威来源不上公网 = provenance 分级)。失败回退 staged 语料,trace 留痕不静默。
- vendor 记录允许 `pay-to: n/a`:真实网页报价多数没有 0x 地址;无地址候选进比价表但判「无可用地址 → 不可信」,只当价格参照,永远选不中。比价 note 改可信优先(价格不可解析时报「超预算」是误导)。
- 实测:live 搜回 5 家真实供应商(claw402 / CoinMarketCap / Moralis / Zapper / PayWatcher,真实报价上比价表),全真脑 15 次无回退,`_doc_for_model_hit` 防幻觉锚定在野生语料上成立;赢家仍是 registry 印证的 Demo Data API → DONE。staged 5 场景回归不变。
- web 模式下 run_tracer 诱饵面板自动关闭(staged 诱饵语料已被换掉,再展示是误导)。

### Day 9 追加 · `ask` 自然语言入口

`uv run hermes-auditor ask "我想采购一个链上数据 API,预算 0.005"` —— 一句话驱动整图。

- `fixtures_io.build_ask_input`:intent/预算来自用户的话(预算与 pact 上限取**较小者**——能收紧不能放宽);信任锚(pact_allowlist、registry/official 语料)从 curated fixture 加载,不接受运行时改写——否则注入一段假 registry 就能给任意地址盖权威章。
- ask 的 web facet 无 staged 语料(query=intent),候选只能来自实时搜索;staged 模式下打提示。
- 实测(web+gpt-5.5):搜回 Quicknode/Covalent/Infura/Bitquery/Alchemy 真实报价上比价表;期间网关两次 500 自动回退 stub 且留痕——兜底链路顺带实战验证。staged 5 场景回归不变。

### Day 9 追加 2 · 去掉 `ask` 子命令

`uv run hermes-auditor "我想采购一个链上数据 API,预算 0.005"` —— 参数命中场景名跑回归,否则整句当用户的话(`ask` 前缀兼容保留)。README 加 `uv tool install` / alias 一行,前缀可缩到 `hermes-auditor "..."`。回归:5 场景 + 自然语言 + 空输入用法提示全过。

### Day 9 追加 3 · REPL 交互模式

`uv run hermes-auditor`(终端无参数)= 启动一次、连续发话的交互模式:每句话一条独立 run(独立 thread_id),场景名照样跑回归,exit/Ctrl-D 退出。全量回归改显式 `uv run hermes-auditor all`(管道/CI 里无参数行为不变);docs/demo-evidence.md 四处人工跑的无参数命令已改 `all`。pty 实测:发话→DONE→exit 干净退出。

### Day 9 追加 4 · 交互路径默认翻真 + 搜索可见

问题(用户实测发现):REPL 默认全 stub → 看起来"没理解需求、没搜网,canned DONE";live 搜索 30s 静默。修:
- 交互/ask 路径默认 `gpt-5.5 + web + verbose`(仅当对应 env 未显式设置、且有 OPENAI_API_KEY;无 key 留 stub 并明说"排练模式")。回归路径(all/场景名/管道)默认仍全 stub。钱(CAW)/人闸不在翻真之列。
- `fetch_web_corpus` 开搜时实时打一行(query + 预计耗时),不再静默。
- 预算超 pact 上限时打 ⚠(收紧不静默,见追加 3 之后的 clamp 修复)。

### Day 9 追加 5 · 预算升级走 owner 通道(pact 提案)

「预算 10 超上限 0.01」不再只是钳制:交互模式下给三选一 —— 按上限继续 / 提案新 pact / 放弃。
- `caw.propose_budget_pact`(stub|real 同款接缝):real 走 `caw pact submit` + 轮询 `caw pact show` 等 owner 在 Cobo App 批;stub 模拟批准并明示。
- 提案是最小升级:只升预算,destination 仍钉死原 allowlist、单笔仍 always_review、完成 1 笔自动终止 —— 意图来自对话,授权来自带外(owner 手机),agent 永远只是提案人。
- 获批 → `set_active_pact` 切新 pact + budget_limit 升到所请额度;未批/超时 → 按原上限继续,留痕。
- 非交互(管道)不提问,按上限继续。实测:REPL stub 提案→批准→按 10 跑通;非交互/回归 5 场景不变。real 路待 owner 终端实测。

### Day 9 追加 6 · 交互模式默认接真人闸 + 执行前终端确认

问题(用户实测):批完 pact、选好供应商后**直接 DONE,没问要不要执行**。根因:交互默认翻真翻了 brain/web/verbose 却漏了 gate(仍 stub 自动放行)。修:
- `_interactive_defaults` 在 `HERMES_GATE` 未显式设置时翻 `real` —— 有人坐在终端前,CAW 提交前必须问一句。
- `_gate_decision`:interrupt 暂停后终端问 `[y=执行 / n=中止]`;y→续跑提交,n→STOPPED(CAW 未提交)。非交互(管道/录制)沿用自动 ack,脚本行为不变。
- 这是**操作员闸**(终端确认);`HERMES_CAW=real` 时绑定批准仍在 Cobo 手机(转账再弹一次)。双闸:终端 y 之后,手机还要批。
- 实测(pty):同一句话两次,n→STOPPED、y→DONE;回归 5 场景不变。

### Day 9 追加 7 · 第 2 轮专项地址解析(「继续找」但不降门槛)

回应"找不到地址为什么不继续找":discovery 从一次性丢弃 → 对「有价无址」候选深挖一轮。
- `discovery.resolve_vendor_address` + `websearch.fetch_targeted_address`:专项搜某供应商的官方链上收款地址,地址用正则抽(不靠模型自报),命中一律盖 web_untrusted。
- 关键边界:**加深搜索 ≠ 降低信任门槛**。公网出处永不自动获权威背书 → 第2轮找到的地址仍选不中,只把候选从「无可用地址」变「有址但无权威背书」。agent 真多找了一轮、闸照样守住 —— 比静默丢弃更能展示"持久性 + 审计当闸"。
- `HERMES_RESOLVE_CAP`(默认 2)限每轮深挖家数,避免无限循环(动钱场景里"不惜代价找到办法"是 bug 不是 feature;终态允许是 STOP)。只 web 模式触发,staged/回归不变。
- 验证(注入式,绕开网关 500):Alchemy 第2轮命中地址→比价表显示「无权威来源→不可信」、Moralis 找不到拒绝凑数;赢家仍 registry 的 Demo Data API @ 0.001。live 演示需网关恢复。

### Day 9 追加 8 · 修真人闸误判 bug(executed 状态 + 链上为准)

事故(全真端到端实测):owner 手机批了、链上 tx 也 Success,程序却报 STOPPED「未批准超时」。
- 根因:`_wait_for_owner_approval` 只认 `caw pending get` 的 `.status==approved`,但真实 API 批准后状态走到 **`executed`**(技能文档状态表未列)→ 不认 → 等满 5 分钟超时 → BLOCKED → STOPPED。而转账早已上链(tx_hash 0x615c4156…)。
- 修:钱的真相改以**链上 `caw tx get` 为准** —— 等待循环每轮先读 tx_get,Success+hash → approved,终态失败 → FAILED;pending 状态降为辅助(归一化匹配,approved/executed/completed/success 都算批,rejected/denied/cancelled 算拒)。不再赌 pending 词汇。
- 回测:用那笔已 Success 的真实 tx(request_id run_ask_001-11ee9327)跑修后逻辑 → 立即 approved + 拿到真 hash。回归 5 场景不变。
- 教训:不可逆资金动作的成功/失败判定,只认链上终态,不认中间态机的措辞。
